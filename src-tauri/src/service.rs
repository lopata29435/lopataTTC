...
pub fn install_service(binary: &Path, toml_text: &str) -> Result<()> {
    ...
    if std::fs::write(&cfg, toml_text).is_err() {
        ...
        write_file_elevated(&cfg, toml_text).context("write service config (elevated)")?;
    }
...
fn write_file_elevated(_path: &Path, _content: &str) -> Result<()> {
    Err(anyhow::anyhow!(
        "elevated write not implemented on non-Windows"
    ))
}
...
pub fn query_service() -> ServiceStatus {
    ...
    for name in names {
        ...
        if let Ok(o) = out {
            ...
            last_raw = txt.clone();
            if !txt.is_empty()
                && !txt.to_lowercase().contains("does not exist")
                && o.status.success()
            {
...
pub fn query_service() -> ServiceStatus {
    ServiceStatus {
        installed: false,
        running: false,
        raw: "service mode is Windows-only".into(),
    }
}
